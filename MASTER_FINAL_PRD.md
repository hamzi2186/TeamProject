# T Rex — Master Product Requirements Document (PRD) v8

*Reconciled with instructor architecture decisions: unified shared Supabase/PostgreSQL database, single Alembic migration chain under `backend/alembic/versions/`, trunk-based integration on shared `main`, exclusive TPI boundary, and modular engine frontend/backend ownership.*
...
3. **Root authentication + basic Docker + shared Supabase/PostgreSQL conventions are shared platform foundations.** ONE shared Supabase/PostgreSQL database project for all engines. No engine may launch a private database container or maintain an independent migration chain. All schema migrations reside in `backend/alembic/versions/`. The Scraper owner is responsible for creating this initial foundation and pushing it first so all teams can pull and build on it.
...
21. **The entire team integrates on the shared `main` branch.** Do not create long-lived or permanent engine feature branches in configuration, CI, or deployment.
22. **Every customer-facing engine must own its own `frontend/` and `backend/`.** Root `frontend/` is platform shell, navigation, and integration only; it must not house private engine feature implementations.
...
- Root database connection and centralized Alembic migrations under `backend/alembic/versions/`
...
The first shared foundation is implemented and pushed by the Scraper owner before engine teams build their own modules on the shared main branch.
...
- Pydantic v2
- SQLAlchemy 2.x
- One centralized Alembic migration chain under `backend/alembic/versions/`
- `httpx` for HTTP integrations

## Database / Vector Storage

- Supabase-hosted PostgreSQL (single shared database project for the entire team; no separate engine database containers)
- pgvector extension
- All operational data in the shared PostgreSQL database
- Vectors in pgvector
...
- root Auth/JWKS,
- the same Supabase/PostgreSQL project (no private engine database containers),
- the shared Alembic migration chain in `backend/alembic/versions/`,
- shared Redis in integrated mode,
- shared TPI,
- another engine's **public API / shared contract** where the product requires it.
...
Root backend signs access tokens using RS256 with an RSA-2048 keypair, exposing its public verification keys at `GET /.well-known/jwks.json`. Independent engine backends do not import that Python function. They verify the same signed token contract through this HTTP JWKS endpoint and expose an equivalent local dependency that resolves the same `AuthenticatedUser` shape.
...
- Modern password hashing using Argon2id or bcrypt with per-user salt and work factors. Unsalted single-iteration algorithms (such as SHA-256) are strictly prohibited.
...
HubSpot OAuth connections and encrypted tokens are strictly scoped to the authenticated `user_id`. Copying or sharing OAuth credentials or encrypted tokens across users to resolve connection or testing errors is prohibited.
...
Downstream engines (Calling, SMS, Mailer) must retrieve Client KB context via `POST /api/v1/internal/client-kb/search` authenticated by the `X-Scraper-Service-Token` header and passing the resolved `user_id` and `knowledge_base_id`. Direct database queries or private source imports are forbidden.
...
## 19.1 Purpose and Phased Architecture

The Agent Engine is the customer-facing conversational assistant explaining T Rex, grounded in approved platform documentation.

It is structured into three continuous capability phases:

### Phase 1: Knowledge Ingestion Foundation
- Ingests markdown documentation from approved directories.
- Computes SHA-256 content hashes to detect document changes.
- Skips unchanged files to ensure idempotent re-indexing.
- Chunks updated documents and embeds passages via TPI Jina embeddings (`agent-jina` provider).
- Persists document records in `agent_documents` and chunk vectors in `agent_knowledge_chunks` (1024-dimension pgvector).

### Phase 2: Grounded RAG Generation
- Accepts user questions, computes query embeddings via TPI, and searches `agent_knowledge_chunks` using cosine distance.
- Enforces a configurable semantic relevance threshold.
- When retrieved chunks fall below the configured threshold, returns a deterministic fallback message without calling the text LLM.
- When relevant context is found, synthesizes a grounded answer via TPI Groq LLM with citations to source documents.
- Stateless endpoint `/api/v1/agent/assistant/ask` is retained for backward compatibility.

### Phase 3: Persistent Conversational Assistant
- Maintains persistent, multi-turn conversation sessions stored in `agent_conversations` and `agent_messages`.
- All conversation data is strictly scoped to the authenticated `user_id`.
- Enforces bounded conversation history for prompt construction.
- Supports conversation listing, retrieval, renaming, deletion (with cascading message deletion), and message posting.
- Persists source citation metadata alongside assistant responses.

### Functional Guardrail
The Agent Engine is strictly an informational documentation assistant. It has no autonomous tool-execution capabilities and cannot initiate phone calls, dispatch SMS messages, send emails, or modify database records.
...
Provide administrative tools and endpoints:

```http
POST /api/v1/agent/admin/ingest
GET  /api/v1/agent/admin/sources
```
...
6. chunk text with configurable token bounds and overlap,
7. embed via TPI Jina (`agent-jina`),
8. upsert `agent_documents` and `agent_knowledge_chunks`,
9. store version and last indexed timestamp.
...
Customers:
- conversational chat and viewing citations only.

Developer/team_member/admin:
- may trigger documentation ingestion and view indexed source status.
...
- LLM provider(s)
- Embeddings:
  - Jina primary (with strict credential isolation between `JINA_API_KEY` for Scraper and `AGENT_JINA_API_KEY` for Agent)
...
Provider secrets live only in TPI secret configuration/storage. TPI must maintain separate configuration and credentials for Agent embeddings (`AGENT_JINA_API_KEY`) and Scraper embeddings (`JINA_API_KEY`). Agent embedding operations must fail closed and never fall back to Scraper credentials.
...
```http
# Documentation Ingestion & Administration
POST   /api/v1/agent/admin/ingest
GET    /api/v1/agent/admin/sources

# Conversational Assistant (Phase 3)
...
# Stateless Assistant (Phase 2 Backward Compatibility)
POST   /api/v1/agent/assistant/ask

# Semantic Chunk Retrieval
POST   /api/v1/agent/retrieval/search
```
...
Owned by Agent Engine (migration `20260916_0005`).
...
## 24.19B `agent_knowledge_chunks`

Owned by Agent Engine (migration `20260916_0005`).

```text
id UUID PK
document_id UUID FK -> agent_documents.id ON DELETE CASCADE
module_key TEXT
source_path TEXT
chunk_index INT
...
embedding_provider TEXT
embedding_model TEXT
embedding_dimension INT CHECK (embedding_dimension = 1024)
...
## 24.19C `agent_conversations`

Owned by Agent Engine (migration `20260917_0007`).
...
## 24.19D `agent_messages`

Owned by Agent Engine (migration `20260917_0007`).

```text
id UUID PK
conversation_id UUID FK -> agent_conversations.id ON DELETE CASCADE
role VARCHAR(32) CHECK (role IN ('user', 'assistant'))
content TEXT
...
The Root platform shell routes and reverse-proxies `/agent/*` to the Agent frontend service. The Agent frontend reuses the active Root platform authentication session; no independent Agent login exists. Access tokens must NEVER be transferred via URL query parameters (`?token=<JWT>`). In the browser SPA deployment, the client reuses the active web session (e.g. `sessionStorage`).
...
agent-engine/frontend/src/features/agent/
```
...
4. All engines use the same integrated Supabase/PostgreSQL project and the single Alembic migration chain under `backend/alembic/versions/`. No per-engine databases or independent migration chains are allowed.
...
10. Shared schema changes require coordination and must be placed in the single Alembic migration chain under `backend/alembic/versions/`.
11. Never commit `.env`.
...
16. Communication engines use the Scraper Client KB through its internal retrieval contract (`POST /api/v1/internal/client-kb/search` authenticated by `X-Scraper-Service-Token`).
...
21. The entire team integrates on the shared `main` branch using trunk-based integration; do not create long-lived or permanent engine feature branches in configuration or deployment.
...
11. Create shared Alembic migrations under `backend/alembic/versions/` before service logic depends on tables.
12. The entire team integrates on the shared `main` branch using trunk-based development; do not create long-lived or permanent engine feature branches in configuration or deployment.
...
- **Auth is shared.**
- **Supabase/PostgreSQL is shared for the integrated MVP; no engine may run a separate database container.**
- **All database migrations belong to the single shared Alembic migration chain under `backend/alembic/versions/`.**
...
This is the reconciled **Master PRD v8**.
...
- same integrated Supabase/PostgreSQL project (no private engine database containers),
- single centralized Alembic migration chain under `backend/alembic/versions/`,
